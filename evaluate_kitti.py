...
def main(args):
    predicted_disp_path = (args.predicted_disp_fn)
    gt_path = args.gt_path

    assert os.path.isfile(predicted_disp_path), "{} is not a file.".format(predicted_disp_path)
    assert os.path.isdir(gt_path), "{} is not a dir".format(gt_path)
    ...
    for i in range(num_samples):
        ...
        disp_mask = np.logical_and(gt_disp > 0, pred_disp > 0)
        ...
        d1_all[i] = 100 * bad_pixels.sum() / disp_mask.sum()

        abs_rel[i], sq_rel[i], rms[i], log_rms[i], a1[i], a2[i], a3[i] = compute_errors(gt_depth[disp_mask], pred_depth[disp_mask])

    print("ABS REL: ", abs_rel.mean())
    print("SQ REL: ", sq_rel.mean())
    print("RMSE: ", rms.mean())
    print("RMSE LOG: ", log_rms.mean())
    print("delta < 1.25", a1.mean())
    print("delta < 1.25 ** 2", a2.mean())
    print("delta < 1.25 ** 3", a3.mean())

if __name__ == "__main__":
    parser = argparse.ArgumentParser()

    parser.add_argument("predicted_disp_fn", type=str)
